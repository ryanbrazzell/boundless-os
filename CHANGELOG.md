...
https://buildermethods.com/boundless-os
...
Version 2.1 implemented a round of significant changes to how things work in Boundless OS.  Here is a summary of what's new in version 2.1.0:

### TL;DR

Here's the brief overview. It's all detailed below and the [docs](https://buildermethods.com/boundless-os) have been updated to reflect all of this.
...
When the config option standards_as_claude_code_skills is true, this will convert all of your standards into Claude Code Skills and _not_ inject references to those Standards like Boundless OS normally would.

2.1 also provides a Claude Code command, `improve-skills` which you **definitely should** run after installing Boundless OS in your project with the skills option turned on.  This command is designed to improve and rewrite each of your Claude Code Skills descriptions to make them more useable and discoverable by Claude Code.
...
When set to false, and when using Claude Code, you can still run Boundless OS commands in Claude Code, and instead of delegating most tasks to subagents, Claude Code's main agent will execute everything.
...
Now we configure Boundless OS using these boolean options in your base ~/boundless-os/config.yml:

claude_code_commands: true/false
use_claude_code_subagents: true/false
boundless_os_commands: true/false

The benefits of this new configuration approach are:

- Now you can use Boundless OS with Claude Code *with* or *without* delegating to subagents.  (subagents bring many benefits like context efficiency, but also come with some tradeoffs—higher token usage, less transparency, slower to finish tasks).

- Before, when you had *both* single-agent and multi-agent modes enabled, your project's boundless-os/commands/ folder ended up with "multi-agent/" and "single-agent/" subfolders for each command, which is confusing and clumsy to use.  Now in 2.1.0, your project's boundless-os/commands/ folder will not have these additional "modes" subfolders.
...
While some users use all of Boundless OS' workflow for everything, many have been picking the parts they find useful and discarding those that don't fit their workflow—AS THEY SHOULD!

2.1.0 establishes this as a core principle of Boundless OS:  You can use as much or as little of it as you want!

With that in mind, we've moved from 4 to 6 different phases of development that can _potentially_ be powered by Boundless OS:
...
Now whenever you need to upgrade your Boundless OS project installation (to a new version or to push configuration changes or standards changes to a project), now when you run project-install.sh or project-update.sh, the system will:
...
- Updated base installation update options to include a "Full update" option, which is the easiest way to pull and update the latest Boundless OS stuff (default profile, scripts) without losing your base installation's custom profiles.
...
- Fixed multi-agent-mode not installing the roles/ files in the project boundless-os folder.
...
- Ensured Claude Code subagents are installed to a project's .claude/agents/boundless-os and not sub-folders within that.
...
Boundless OS 2.0 is a major new release that brings several core architectural changes and improvements.
...
[this page](https://buildermethods.com/boundless-os/version-2) documents:

- The new features in Boundless OS 2.0
- Architectural changes in 2.0
- What changed from 1.x
- Updating guide

[The Boundless OS docs](https://buildermethods.com/boundless-os) also received a complete overhaul and expansion.  It's now broken out into multiple pages that document every detail of how to install, use and customize Boundless OS.
...
Earlier versions added a decisions.md inside a project's .boundless-os/product/.  In practice, this was rarely used and didn't help future development.
...
The way Boundless OS gets installed is structured differently from prior versions.  The new system works as follows:
...
**"Base installation"**
- Installs all of the Boundless OS files to a location of your choosing on your system where they can be customized (especially your standards) and maintained.
...
To install the Boundless OS base installation,

1. cd to a location of your choice (your system's home folder is a good choice).

2. Run one of these commands:
  - Boundless OS with Claude Code support:
  `curl -sSL https://raw.githubusercontent.com/buildermethods/boundless-os/main/setup/base.sh | bash -s -- --claude-code`
  - Boundless OS with Cursor support:
  `curl -sSL https://raw.githubusercontent.com/buildermethods/boundless-os/main/setup/base.sh | bash -s -- --cursor`
  - Boundless OS with Claude Code & Cursor support:
  `curl -sSL https://raw.githubusercontent.com/buildermethods/boundless-os/main/setup/base.sh | bash -s -- --claude-code --cursor`

3. Customize your /standards (just like earlier versions)

**Project installation**

- Now each project codebase gets it's own self-contained installation of Boundless OS.  It no longer references instructions or standards that reside elsewhere on your system.  These all get installed directly into your project's .boundless-os folder, which brings several benefits:
  - No external references = more reliable Boundless OS commands & workflows.
...
Your project installation command will be based on where you installed the Boundless OS base installation.
- If you've installed it to your system's home folder, then your project installation command will be `~/.boundless-os/setup/project.sh`.
- If you've installed it elsewhere, your command will be `/path/to/boundless-os/setup/project.sh`
...
If (for whatever reason) you didn't install the base installation, you can still install Boundless OS directly into a project, by pulling it directly off of the public github repo using the following command.
...
`curl -sSL https://raw.githubusercontent.com/buildermethods/boundless-os/main/setup/project.sh | bash -s -- --no-base --claude-code --cursor`

### Boundless OS config.yml

When you install the Boundless OS base installation, that now includes a config.yml file.  Currently this file is used for:
- Tracking the Boundless OS version you have installed
...
- By default, a new installation of Boundless OS into a project will copy its instructions and standards from your base installation's /instructions and /standards.
- You can define additional project types by doing the following:
  - Setup a folder (typically inside your base installation's .boundless-os folder, but it can be anywhere on your system) which contains /instructions and /standards folders (copy these from your base install, then customize).
...
- setup-claude-code.sh (now you add --claude-code flag to the install commands or enable it in your Boundless OS config.yml)
- setup-cursor.sh (now you add --cursor flag to the install commands or enable it in your Boundless OS config.yml)

Claude Code Boundless OS commands now should _not_ be installed in the `~/.boundless-os/.claude/commands` folder.  Now, these are copied from ~/.boundless-os/commands into each project's `~/.claude/commands` folder (this prevents duplicate commands showing in in Claude Code's commands list).  The same approach applies to Claude Code subagents files.
...
2. Navigate to your home directory (or whichever location you want to have your Boundless OS base installation)
...
`curl -sSL https://raw.githubusercontent.com/buildermethods/boundless-os/main/setup/base.sh | bash -s -- --overwrite-instructions --claude-code --cursor`

4. If your ~/.claude/commands contain Boundless OS commands, remove those and copy the versions that are now in your base installation's commands folder into your _project's_ `.claude/commands` folder.

5. Navigate to your project. Run your project installation command to install Boundless OS instructions and standards into your project's installation. If your Boundless OS base installation is in your system's home folder (like previous versions), then your project installation will be: `~/.boundless-os/setup/project.sh`
...
- All agents follow Boundless OS conventions (branch naming, commit messages, file templates)
...
### Added
- Initial release of Boundless OS framework
...
[1.4.1]: https://github.com/buildermethods/boundless-os/compare/v1.4.0...v1.4.1
[1.4.2]: https://github.com/buildermethods/boundless-os/compare/v1.4.1...v1.4.2
[1.4.0]: https://github.com/buildermethods/boundless-os/compare/v1.3.1...v1.4.0
[1.3.1]: https://github.com/buildermethods/boundless-os/compare/v1.3.0...v1.3.1
[1.3.0]: https://github.com/buildermethods/boundless-os/compare/v1.2.0...v1.3.0
[1.2.0]: https://github.com/buildermethods/boundless-os/compare/v1.1.0...v1.2.0
[1.1.0]: https://github.com/buildermethods/boundless-os/compare/v1.0.0...v1.1.0
[1.0.0]: https://github.com/buildermethods/boundless-os/releases/tag/v1.0.0
