...
# =============================================================================
# Boundless OS Common Functions
# Shared utilities for Boundless OS scripts
# =============================================================================
...
process_workflows() {
    ...
    while IFS= read -r workflow_ref; do
        ...
        if [[ -f "$workflow_file" ]]; then
            ...
            local warning_msg="⚠️ This workflow file was not found in your Boundless OS base installation at ~/boundless-os/profiles/$profile/workflows/${workflow_path}.md"
...
process_phase_tags() {
    ...
    # Find all PHASE tags: {{PHASE X: @boundless-os/commands/path/to/file.md}}
    ...
    while IFS= read -r phase_ref; do
        ...
        if [[ "$mode" == "embed" ]]; then
            ...
            # Extract: {{PHASE 1: @boundless-os/commands/plan-product/1-product-concept.md}}
            # To get: PHASE 1, plan-product/1-product-concept.md, "Product Concept"

            local phase_label=$(echo "$phase_ref" | sed 's/{{//' | sed 's/:.*$//')  # "PHASE 1"
            local file_ref=$(echo "$phase_ref" | sed 's/.*@boundless-os\/commands\///' | sed 's/}}$//')  # "plan-product/1-product-concept.md"
...
get_project_config() {
    ...
    get_yaml_value "$project_dir/boundless-os/config.yml" "$key" ""
}
...
validate_base_installation() {
    if [[ ! -d "$BASE_DIR" ]]; then
        print_error "Boundless OS base installation not found at ~/boundless-os/"
        echo ""
        print_status "Please run the base installation first:"
        echo "  curl -sSL https://raw.githubusercontent.com/buildermethods/boundless-os/main/scripts/base-install.sh | bash"
        echo ""
        exit 1
    fi
...
check_not_base_installation() {
    if [[ -f "$PROJECT_DIR/boundless-os/config.yml" ]]; then
        if grep -q "base_install: true" "$PROJECT_DIR/boundless-os/config.yml"; then
            echo ""
            print_error "Cannot install Boundless OS in base installation directory"
            echo ""
            echo "It appears you are in the location of your Boundless OS base installation (your home directory)."
            echo "To install Boundless OS in a project, move to your project's root folder:"
            echo ""
...
load_base_config() {
    ...
    BASE_AGENT_OS_COMMANDS=$(get_yaml_value "$BASE_DIR/config.yml" "boundless_os_commands" "false")
...
load_project_config() {
    ...
    PROJECT_AGENT_OS_COMMANDS=$(get_project_config "$PROJECT_DIR" "boundless_os_commands")
...
validate_config() {
    ...
    if [[ "$claude_code_commands" != "true" ]] && [[ "$boundless_os_commands" != "true" ]]; then
        print_error "At least one of 'claude_code_commands' or 'boundless_os_commands' must be true"
        exit 1
    fi
...
write_project_config() {
    ...
    local boundless_os_commands=$5
    local standards_as_claude_code_skills=$6
    local dest="$PROJECT_DIR/boundless-os/config.yml"
...
#
# To change these settings, run ~/boundless-os/scripts/project-update.sh to re-compile your project with the new settings.
# ================================================
profile: $profile
claude_code_commands: $claude_code_commands
use_claude_code_subagents: $use_claude_code_subagents
boundless_os_commands: $boundless_os_commands
...
create_standard_skill() {
    ...
    # Create skill directory (directly in .claude/skills/, not in boundless-os subfolder)
    ...
    # Prepend boundless-os/ to the standards file path for the file reference
    local standard_file_path_with_prefix="boundless-os/$standards_file"
